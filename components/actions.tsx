...
export function ConsultButton({locale, children, className='button secondary'}: {locale: Locale; children: React.ReactNode; className?: string}) {
  return <button className={className} onClick={() => {track('hvac_consultation_clicked',locale);window.dispatchEvent(new CustomEvent('open-consultation'))}}>{children}<ArrowRight size={17}/></button>;
}
